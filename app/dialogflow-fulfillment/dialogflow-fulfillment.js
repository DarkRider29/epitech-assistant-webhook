...
class WebhookClient {
  ...
  handleRequest(handler) {
    if (typeof handler === 'function') {
      let result = handler(this, this.assistant);
    ...
    if (handler.get(this.intent)) {
      let result = handler.get(this.intent)(this, this.assistant);
      // If handler is a promise use it, otherwise create use default (empty) promise
      let promise = Promise.resolve(result);
      return promise.then(() => this.send_());
    } else if (handler.get(null)) {
      let result = handler.get(null)(this, this.assistant);
      // If handler is a promise use it, otherwise create use default (empty) promise
      let promise = Promise.resolve(result);
      return promise.then(() => this.send_());
    } else {
